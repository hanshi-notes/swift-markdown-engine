...
- **Don't add external dependencies to the core `MarkdownEngine`
  ...
  `SyntaxHighlighter`, `LatexRenderer`) instead. The package has no
  external dependencies at all and no bundled bridge products. Adding
  either is a bigger call — make the case in the PR description.
